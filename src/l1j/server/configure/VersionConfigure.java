...
public class VersionConfigure {
	...
	public void load(){
		try {
			...
			String keyText						= new String(settings.getProperty("FIRST_KEY", StringUtil.EmptyString).getBytes(CharsetUtil.ISO_8859_1_STR), CharsetUtil.BIG5_STR);
			...
			for (int i=0; i<FIRST_KEY.length; i++) {
				if (i == 0) {
					FIRST_KEY[i] = (byte) ((array.length + 2) & 0xff);// 패킷 길이
				} else if (i == 1) {
			...
			SERVER_NUMBER						= Integer.parseInt(settings.getProperty("SERVER_NUMBER", "1"));
			CLIENT_VERSION						= Long.parseLong(settings.getProperty("CLIENT_VERSION", "2211231701"));
			LIBCOCOS_VERSION					= Long.parseLong(settings.getProperty("LIBCOCOS_VERSION", "2211231701"));
