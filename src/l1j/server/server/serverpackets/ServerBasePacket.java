...
public abstract class ServerBasePacket {
	...
	protected void writeS(String text) {
		try {
			if (text != null) {
				_bao.write(text.getBytes(CharsetUtil.BIG5_STR));
			}
		} catch (Exception e) {
			_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
		}
	...
	protected void writeSS(String text) {
		try {
			if (text != null) {
				byte[] test = text.getBytes(CharsetUtil.BIG5_STR);
	...
	protected void writeS2(String text) {
		try {
			if (text != null && !text.isEmpty()) {
				byte[] name = text.getBytes(CharsetUtil.BIG5_STR);
	...
	protected void writeStringWithLength(String s) {
	    try {
	        if (StringUtil.isNullOrEmpty(s)) {
	            ...
	            byte[] data = s.getBytes(CharsetUtil.BIG5_STR);
