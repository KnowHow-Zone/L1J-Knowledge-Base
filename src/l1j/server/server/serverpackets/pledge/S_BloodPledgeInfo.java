...
public class S_BloodPledgeInfo extends ServerBasePacket {
	...
	public S_BloodPledgeInfo(int pledge_id) {
		...
		try {
			...
			int i = 0;
			for (byte b : clan.getAnnouncement().replaceAll(StringUtil.MinusString, StringUtil.EmptyString).getBytes(CharsetUtil.BIG5_STR)) {
				text[i++] = b;
			}
			writeByte(text);
	...
	public S_BloodPledgeInfo(String name, String notes){
		...
		if (notes.length() != 0) {
			int i = 0;
			try {
				for (byte b : notes.getBytes(CharsetUtil.BIG5_STR)) {
					text[i++] = b;
				}
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
