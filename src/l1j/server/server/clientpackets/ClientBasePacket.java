...
public abstract class ClientBasePacket {
	...
	public String readString() {
		...
		String s = null;
		try {
			s = new String(_decrypt, _off, length, CharsetUtil.BIG5_STR);//EUC-KR
			s = s.substring(0, s.indexOf('\0'));
			_off += s.getBytes(CharsetUtil.BIG5_STR).length + 1;
		} catch (Exception e) {
			e.printStackTrace();
		}
		return s;
	...
	public String readRS() {
		String s = null;
		try {
			s = new String(_decrypt, _off, _decrypt.length - _off, CharsetUtil.BIG5_STR);// EUC-KR
	...
	public String readS(int len) {
		String s = null;
		try {
			s = new String(_decrypt, _off, len, CharsetUtil.BIG5_STR);
			_off += s.getBytes(CharsetUtil.BIG5_STR).length;
		} catch (Exception e) {
			e.printStackTrace();
		}
		return s;
	...
	public String readS2(int length) {
		String s = null;
		try {
			s = new String(_decrypt, _off, length, CharsetUtil.BIG5_STR);
			s = s.substring(0, s.indexOf('\0'));
			_off += s.getBytes(CharsetUtil.BIG5_STR).length + 1;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String readSAccount() {
		String s = null;
		try {
			s = new String(_decrypt, _off, _decrypt.length - _off, CharsetUtil.BIG5_STR);
			String target = "?";
		...
		try{ 
			...
			do { 
				...
				/** 한글 **/ 
					...
					t[1] = _decrypt[start]; 
					test.append(new String(t, 0, 2, CharsetUtil.BIG5_STR)); 
				} else {
					test.append(new String(_decrypt, start, 1, CharsetUtil.BIG5_STR));/** 영문&숫자 **/ 
				}
				start+=2; 
				loc-=2; 
			} while (0 < loc);
    ...
    public String readS2() {
        ...
        try {
            ...
            s = new String(this._decrypt, this._off, size, CharsetUtil.BIG5_STR);
            this._off += size;
        } catch (Exception e) {
        	e.printStackTrace();
        }
    ...
    public String readS22() {
        ...
        try {
            int size = this._decrypt[this._off++] & 0xFF;
            s = new String(this._decrypt, this._off, size, CharsetUtil.MS_950_STR);
            this._off += size;
        } catch (Exception e) {
        	e.printStackTrace();
        }
