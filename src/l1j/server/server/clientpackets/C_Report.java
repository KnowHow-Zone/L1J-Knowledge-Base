...
public class C_Report extends ClientBasePacket {
	...
	void createSecondPassword() {
		...
		try {
			password = new String(pw, 0, passlength, CharsetUtil.BIG5_STR);
		} catch (Exception e) {
		}
	...
	void changeSecondPassword() {
		...
		try {
			password	= new String(pw, 0, passlength, CharsetUtil.BIG5_STR);
			npassword	= new String(npw, 0, npasslength, CharsetUtil.BIG5_STR);
		} catch (Exception e) {
		}
	...
	void changePledgeName() throws UnsupportedEncodingException {
		...
		int numOfNameBytes = newClanName.getBytes(CharsetUtil.BIG5_STR).length;
